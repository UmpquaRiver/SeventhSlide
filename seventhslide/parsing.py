...
# Canonical inline-size tags (<size=NN>…</size>) as stored by the song editors —
# see convert_size_tags in seventhslide.render_safe. Passed through (normalized) by the sanitizer.
_SIZE_OPEN_TOKEN_RE = re.compile(r'<size=(\d{1,3})>\Z', flags=re.IGNORECASE)
...
def _sanitize_inline_html(text: str) -> str:
    """Keep only <b>/<i>/<u> (strong/em normalized) and <size=NN>; escape other tags."""
    if not text:
        return ''
    ...
    def repl(m: re.Match) -> str:
        raw = m.group(0)
        size_open = _SIZE_OPEN_TOKEN_RE.match(raw)
        if size_open:
            return f"<size={size_open.group(1)}>"
        if raw.lower() == '</size>':
            return '</size>'
...
def _safe_xml_parse(path):
    """Parse XML while rejecting DOCTYPE/DTD (XXE / entity-expansion)."""
...
# OpenLyrics/OpenSong: <chord name="X"> -> [X]
_CHORD_OPEN_RE = re.compile(r'<(?:\w+:)?chord\b[^>]*name=[\'"]([^\'"]+)[\'"][^>]*>')
_CHORD_CLOSE_RE = re.compile(r'</(?:\w+:)?chord>')


def _build_block_text(v_node, ns):
    """Text of one <verse>: <lines> with chords as [Name], or flattened text."""
    lines = []
...
def _parse_openlyrics_song(s, ns):
    """Parse a single OpenLyrics <song> node into a Song (title, verse-tagged lyrics,
    authors, first songbook, key)."""
    title = (_ns_findtext(s, 'title', ns) or s.findtext('.//name') or 'Untitled').strip()

    lyrics_parts = []
    verse_nodes = _ns_findall(s, 'verse', ns)
    if verse_nodes:
        for v in verse_nodes:
            # Verse name e.g. "v1"/"c1"/"o1" -> normalized "Verse:1" style label.
            ...
            if base:
                ...
                label = vname
            block_text = _build_block_text(v, ns)
            if block_text:
    ...
    # Authors — try the namespaced path first, then a bare path.
    anodes = s.findall(f'.//{ns}authors/{ns}author') if ns else []
    if not anodes:
        anodes = s.findall('.//authors/author')
    auth_parts = [t for t in ((a.text or '').strip() for a in anodes) if t]
    ...
    return Song(title, lyrics, verse_order=None, authors=auth_parts,
                songbook_name=sb_name, songbook_entry=sb_entry, key=song_key)
...
def parse_song_file(path):
    ...
    if song_nodes:
        return [_parse_openlyrics_song(s, ns) for s in song_nodes]
    ...
    lyrics = '\n\n'.join(parts)
    return [Song(title.strip(), lyrics)]


# ---------------------- Bible XML ----------------------

def _parse_bible_books(root, book_tag, chap_tag, verse_tag, book_attr, chap_attr, verse_attr):
    """Walk a book/chapter/verse tree into a flat verse-dict list."""
    verses = []
    for b_node in root.findall(book_tag):
        b_name = b_node.get(book_attr) or "Unknown Book"
        for c_node in b_node.findall(chap_tag):
            c_num = int(c_node.get(chap_attr) or 0)
            for v_node in c_node.findall(verse_tag):
                v_num = int(v_node.get(verse_attr) or 0)
                text = v_node.text or ""
                if text:
                    verses.append({'book': b_name, 'chapter': c_num,
                                   'verse': v_num, 'text': text.strip()})
    return verses
...
def parse_bible_file(path):
    ...
    if root.tag.upper() == 'XMLBIBLE':          # Zefania: XMLBIBLE / BIBLEBOOK / CHAPTER / VERS
        verses = _parse_bible_books(root, 'BIBLEBOOK', 'CHAPTER', 'VERS',
                                    'bname', 'cnumber', 'vnumber')
    elif root.tag.lower() == 'bible':           # OpenSong: bible / b / c / v
        verses = _parse_bible_books(root, 'b', 'c', 'v', 'n', 'n', 'n')
    else:
        # Any other root tag is unsupported: only Zefania and OpenSong are recognized,
        # so `verses` stays empty and the caller surfaces it.
        verses = []
